...
const Sidebar = ({ sections }: SidebarProps) => {
  ...
  return (
    <>
      {/* Mobile toggle with improved design */}
      <div className="block lg:hidden mb-6">
        <button
          type="button"
          className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium text-foreground bg-gradient-to-r from-secondary to-secondary/80 rounded-lg hover:from-secondary/90 hover:to-secondary/70 transition-all duration-200 shadow-sm border border-border/50"
      ...
      <div
        className={cn(
          "lg:block border-r border-border/60 lg:w-72 pr-4 pb-10 lg:pb-0 bg-background/80 backdrop-blur-sm",
          isMobileOpen ? "block" : "hidden"
        )}
      >
        <div className="lg:sticky lg:top-16 overflow-y-auto py-6 h-[calc(100vh-4rem)] scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent">
          <div className="px-3 py-2 space-y-6">
                    ...
                    {filteredItems.map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          to={item.href}
                          className={cn(
                            "group flex items-center justify-between px-3 py-2.5 text-sm rounded-lg transition-all duration-200 relative overflow-hidden",
                            isActive
                              ? "bg-primary text-white font-medium shadow-md shadow-primary/25 border border-primary/20"
                              : "text-muted-foreground hover:text-foreground hover:bg-secondary/60 border border-transparent hover:border-border/50"
                          )}
                        >
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0 ml-2">
                            {item.language === 'cpp' && (
                              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-xs rounded-md bg-blue-500/15 text-blue-600 border border-blue-500/20">
                                {getLanguageIcon('cpp')}
                                <span className="hidden sm:inline">C++</span>
                              </span>
                            )}
                            {item.language === 'c' && (
                              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-xs rounded-md bg-green-500/15 text-green-600 border border-green-500/20">
                                {getLanguageIcon('c')}
                                <span className="hidden sm:inline">C</span>
                              </span>
                            )}
                          ...
                          {!isActive && (
                            <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 -z-10"></div>
                          )}
                        </Link>
                      );
                    })}
            ...
            <div className="pt-4 border-t border-border/50">
              ...
              <div className="space-y-1">
                <Link
                  to="/quick-start"
                  className="flex items-center gap-2 px-3 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-secondary/60 rounded-lg transition-colors"
                >
